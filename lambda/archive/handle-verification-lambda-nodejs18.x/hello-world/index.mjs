...
export const handler = async (event) => {
  ...
  const successQuery = `UPDATE preorders SET verified = true WHERE id = $1`;
  const failQuery = `UPDATE preorders SET verified = false WHERE id = $1`;

  try {
    // Parse ID from the event body
    const id = JSON.parse(event.id); // Assuming `id` is provided in the event body
    const success = JSON.parse(event.success); // Assuming `success` is provided in the event body
    ...
    result = await pool.query(query, [parsedId]);
